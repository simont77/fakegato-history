...
drive.prototype.writeFile = function(path, name, data, cb) {
    // get folder ID
    debug("getFolder");
    if (auth) {
        getFolder(path,function(err, folder) {
            debug("upload");
            myUploadFile(folder, name, data, cb);
        })
    }
}
drive.prototype.readFile = function(path, name, cb) {
    // get folder ID
    debug("getFolder");
    if (auth) {
        getFolder(path,function(err, folder) {
...
function myDownloadFile(folder, name, cb) {
    ...
    debug("download file", folder, id);
	
	getFileID(folder, name, function(err,file){
		drive.files.export({
			fileId: file.id,
			mimeType: 'application/vnd.google-apps.script+json'
		}).then(function(success){
			cb(null,success.result.webContentLink);
		}).catch(function(fail){
			cb(fail.result.error);
		});
	});
