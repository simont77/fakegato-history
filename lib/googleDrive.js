...
drive.prototype.writeFile = function(path, name, data, mime) {
    // get folder ID
    debug("getFolder");
    if (auth) {
        getFolder(path,function(err, folder) {
            debug("upload");
            uploadFile(folder, name, data, mime);
        })
    }
}
drive.prototype.readFile = function(path, name) {
    // get folder ID
    debug("getFolder");
    if (auth) {
        getFolder(path,function(err, folder) {
...
function uploadFile(folder, name, data, mime) {
    var drive = google.drive('v3');
